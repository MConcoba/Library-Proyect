...
const newBook = async (req, res) => {
  ...
  if (confirmAdmin.role == 'admin') {
    ...
    if (params.copies) {
      ...
      book.countLend.valor = 0;

      let bookExist = await Book.find({
        $or: [{ title: book.title }, { author: book.author }],
      });
      if (bookExist && bookExist.length >= 1)
        return res.status(200).send({ message: 'Book alredy exists' });
      else {
        let bookSave = await book.save();
        if (!bookSave)
          return res.status(200).send({ menssage: 'Error register book' });
        else {
...
const getBoookCoutnLendProcentaje = async (req, res) => {
  const totalBook = await Book.aggregate([
    {
      $group: {
        _id: null,
  ...
  res.status(202).send(booksTotalLend);
};
...
const deleteBook = async (req, res) => {
  ...
  if (confirmAdmin.role == 'admin') {
    ...
    if (!bookDelete) {
      ...
      return res
        .status(202)
        .send({ menssage: 'Book was successfully removed' });
    }
  } else {
    return res.status(200).send({ menssage: 'Access denied' });
  }
  ...
  if (userLogin.role != 'admin') {
    ...
    if (!bookSelected) {
      ...
      if (userBook.books_borrowed == 0) {
        const bookBorrow = await Book.findOneAndUpdate(
          { _id: bookId },
        ...
        if (bookBorrow) {
          const userBorrowing = await User.findOneAndUpdate(
            { _id: isAdmin },
            {
              $push: { books_borrowed: { book: bookSelected._id } },
              $inc: { amount_book_borrowed: 1 },
            },
            { new: true }
          );
          if (userBorrowing) {
        ...
        return res
          .status(200)
          .send({ menssage: 'You have already borrowed this book' });
      }
    } else {
      return res
        .status(200)
        .send({ menssage: 'Does not have books available' });
    }
  } else {
    return res.status(200).send({ menssage: 'Access denied' });
  }
...
const returnBook = async (req, res) => {
  ...
  const userLogin = await User.findOne({ _id: isAdmin });
  if (userLogin.role != 'admin') {
    const bookBorrow = await User.findOne(
      { _id: isAdmin },
      { books_borrowed: { $elemMatch: { book: bookId } } }
    );
    if (!bookBorrow) {
      return res.status(404).send({ menssage: 'ERROR: Book not exists' });
    }
    ...
    if (bookBorrow.books_borrowed != 0) {
      const bookMoficate = await Book.findOneAndUpdate(
        { _id: bookId },
        { $inc: { available: 1 } }
      );
      if (!bookMoficate) {
        ...
        const bookReturn = await User.findOneAndUpdate(
          { _id: bookBorrow.id },
          {
            $pull: { books_borrowed: { book: bookMoficate.id } },
            $inc: { amount_book_borrowed: -1 },
          },
          { new: true }
        );
