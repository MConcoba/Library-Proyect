		...
		if (params.role == 'catedratico' || params.role == 'estudiante') {
			if (params.name && params.lastName && params.role && params.password) {
				...
				if (params.carnet && params.role == 'estudiante') {
					user.userName = util.generateUsername(params.name, params.lastName, params.carnet);
					user.email = util.generateEmail(params.name, params.lastName, params.carnet);
					...
					if (userExist && userExist.length >= 1)
						...
						if (!accountCreated) return res.status(200).send({ menssage: 'Error creating account' });
						else {
							return res.status(202).send(accountCreated);
						}
					}
				} else if (params.dpi && params.role == 'catedratico') {
					user.userName = util.generateUsernameCatedratico(params.name, params.lastName);
					user.email = util.generateEmailCatedratico(params.name, params.lastName);
			...
			return res.status(202).send({ menssage: 'That kind of role does not exist' });
		}
	} else {
...
const login = async (req, res) => {
	...
	if (params.userName) {
		...
		if (!userFind) {
			...
			const compare = await bcrypt.compareSync(params.password, userFind.password);
			if (compare) {
				if (params.getToken) {
		...
		if (!userFind) {
			...
			if (userFind.password == params.password) {
				if (params.getToken) {
					return res.status(202).send({ token: jwt.createToken(userFind), userr: userFind });
				} else {
					userFind.password = undefined;
					return res.status(200).send({ user: userFind });
				}
...
const getUser = async (req, res) => {
	...
	if (confirmAdmin.role == 'admin') {
		...
		if (userFind.id == confirmAdmin.id) {
			return res.status(200).send({ menssage: 'This user cannot be displayed' });
		}
		if (!userFind) {
			return res.status(200).send({ menssage: 'Error the user not exist' });
		} else {
	...
	if (confirmAdmin.role == 'admin') {
		...
		if (!userFind) {
			...
			if (userFind.role == 'estudiante') {
				if (params.name) {
					params.userName = util.generateUsername(params.name, userFind.lastName, userFind.carnet);
					params.email = util.generateEmail(params.name, userFind.lastName, userFind.carnet);
				}
				if (params.lastName) {
					params.userName = util.generateUsername(userFind.name, params.lastName, userFind.carnet);
					params.email = util.generateEmail(userFind.name, params.lastName, userFind.carnet);
				}
				if (params.carnet) {
					params.userName = util.generateUsername(params.name, params.lastName, userFind.carnet);
					params.email = util.generateEmail(params.name, params.lastName, userFind.carnet);
				}
				...
				if (params.name && params.carnet) {
					params.userName = util.generateUsername(params.name, userFind.lastName, params.carnet);
					params.email = util.generateEmail(params.name, userFind.lastName, params.carnet);
				}
				if (params.lastName && params.carnet) {
					params.userName = util.generateUsername(userFind.name, params.lastName, params.carnet);
					params.email = util.generateEmail(userFind.name, params.lastName, params.carnet);
				}
			}
			if (userFind.role == 'catedratico') {
				if (params.name) {
					params.userName = util.generateUsernameCatedratico(params.name, userFind.lastName);
					params.email = util.generateEmailCatedratico(params.name, userFind.lastName);
				}
				if (params.lastName) {
					params.userName = util.generateUsernameCatedratico(userFind.name, params.lastName);
					params.email = util.generateEmailCatedratico(userFind.name, params.lastName);
				}
				if (params.name && params.lastName) {
					params.userName = util.generateUsernameCatedratico(params.name, params.lastName);
					params.email = util.generateEmailCatedratico(params.name, params.lastName);
				}
			}
		}
...
const deleteUser = async (req, res) => {
	...
	if (confirmAdmin.role == 'admin') {
		...
		if (!userDelete) {
			...
			return res.status(202).send({ menssage: 'User was successfully removed' });
		}
	} else {
		return res.status(200).send({ menssage: 'Access denied' });
	}
