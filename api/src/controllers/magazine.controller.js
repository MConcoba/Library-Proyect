	...
	if (confirmAdmin.role == 'admin') {
		...
		if (params.copies >= params.available) {
			...
			if (magazineExist && magazineExist.length >= 1)
				...
				if (!magazineSave) return res.status(200).send({ menssage: 'Error register magazine' });
				else {
	...
	if (confirmAdmin.role == 'admin') {
		if (params.copies && params.available && params.keywords && params.topics) {
			...
			if (!magazineView) {
				...
				const magazineUpdated = await Magazine.findOneAndUpdate({ _id: magazineID }, params, { new: true });
				if (!magazineUpdated) {
					return res.status(404).send({ menssage: 'ERROR: Magazine not exists' });
				} else {
...
const deleteMagazine = async (req, res) => {
	...
	if (confirmAdmin.role == 'admin') {
		...
		if (!magazineDelete) {
			...
			return res.status(202).send({ menssage: 'Magazine was successfully removed' });
		}
	} else {
		return res.status(200).send({ menssage: 'Access denied' });
	}
...
const searchTitle = async (req, res) => {
	...
	const magazineFound = await Magazine.find({ title: { $regex: params.title, $options: 'i' } });
	if (!magazineFound) {
...
const searchKeywords = async (req, res) => {
	...
	const magazineFound = await Magazine.find({ keywords: { $regex: paralabrasClaves, $options: 'i' } });
	if (!magazineFound) {
		return res.status(404).send({ menssage: 'No match found in titles' });
	} else {
		return res.status(202).send(magazineFound);
	...
	if (userLogin.role != 'admin') {
		...
		if (!magazineSelected) {
			...
			const userMagazine = await User.findOne(
				{ _id: userLogin.id },
				{ magazines_borrowed: { $elemMatch: { magazine: magazineSelected._id } } }
			);
			if (userMagazine.magazines_borrowed == 0) {
				...
				return res.status(200).send({ menssage: 'You have already borrowed this magazine' });
			}
		} else {
			return res.status(200).send({ menssage: 'Does not have magazines available' });
		}
	} else {
		return res.status(200).send({ menssage: 'Access denied' });
	}
...
const returnMagazine = async (req, res) => {
	...
	if (userLogin.role != 'admin') {
		...
		if (magazinekBorrow.magazines_borrowed != 0) {
			const magazineMoficate = await Magazine.findOneAndUpdate({ _id: magazineID }, { $inc: { available: 1 } });
			if (!magazineMoficate) {
				return res.status(404).send({ menssage: 'ERROR: Magazine not exists' });
			} else {
			...
			return res.status(200).send({ menssage: 'I did not borrow this magazine' });
		}
	} else {
		return res.status(200).send({ menssage: 'Access denied' });
	}
